...
Nothing here is inferred from the ROM's file name or from a catalogue.
    ...
    python datestamps.py IMAGE --modules DIR

**On this cartridge the image alone answers almost nothing**, and that is the
whole reason for `--modules`.  Thirty-two of the thirty-three executable
modules are `BLZ`-packed, so every compiler and middleware stamp in the code is
inside a compressed stream: run over the shipped image this tool finds two
fragments of one string, and run over the plaintext modules it finds nine
stamps including the build date.  `--modules` scans the plaintext modules after
the image and attributes each hit to its module.

A sequential run of six-digit groups is a numeric table and not a set of dates.
This cartridge carries `040601`, `040602` ... `040631` in order, and June has
thirty days, so the run is arithmetic rather than calendar.  The output prints
them all so the reader can see the run rather than being handed a date list.
...
# `__DATE__` is `Mmm dd yyyy`, and the day is space-padded only when it is a
# single digit: `Jan  1 2008` carries two spaces and `Nov 19 2008` carries one.
# The pattern below required two, so it matched the first shape and missed the
# second -- which on the Tales of Hearts cartridges is the only compiler date
# stamp there is.  Both shapes match now.
...
def main(argv):
    ...
    loc = Locator(rom)
    moddir = argv[argv.index('--modules') + 1] if '--modules' in argv else None
...
def scan_modules(moddir):
    """The same patterns over the plaintext modules, attributed per module."""
    import os
    print('=' * 72)
    print('the same patterns over the BLZ plaintext in %s' % moddir)
    print('=' * 72)
    print('')
    names = sorted(n for n in os.listdir(moddir) if n.endswith('.bin'))
    total = 0
    for label, pat in PATTERNS:
        rows = []
        for n in names:
            b = open(os.path.join(moddir, n), 'rb').read()
            for m in pat.finditer(b):
                rows.append((n[:-4], m.start(), m.group().strip()))
        print('== %s: %d hits over %d modules ==' % (label, len(rows), len(names)))
        for mod, off, text in rows[:60]:
            try:
                t = text.decode('shift_jis', 'replace')
            except Exception:
                t = repr(text)
            print('  %-12s +0x%06X  %s' % (mod, off, t))
        total += len(rows)
        print('')
    print('%d stamps in the plaintext, against what the packed image showed '
          'above.' % total)
    return 0
