...
worth nothing without "out of how many words".
    ...
    python ring_sites.py DIR  --arm --dir [--imm ...]

`--dir` runs the ARM scan over every `.bin` in a directory and prints one row
per module and a totals row.  A Nintendo DS build is not one image: *Tales of
Hearts* is an ARM9, an ARM7 and thirty-one overlays, and on *Tales of
Innocence* 71% of the ARM immediates were in the overlays, so a scan of
`arm9.bin` alone covers under a third of the code and its silence means much
less than it looks like.  The totals row is the number this corpus quotes.
...
def arm_scan(body, base, wanted):
    """The four ARM denominators and the hits, as data rather than as text."""
    ...
    return {
        'imm_total': imm_total, 'thumb_total': th_total,
        'word_total': lit_total, 'load_targets': len(targets),
        'n_arm_ldr': n_arm_ldr, 'n_thumb_ldr': n_thumb_ldr,
        'imm_hits': imm_hits, 'thumb_hits': th_hits, 'lit_hits': lit_hits,
    }


def cmd_dir(root, wanted):
    """Every module in a directory, one row each, then the totals."""
    import os
    names = sorted(n for n in os.listdir(root) if n.endswith('.bin'))
    print('# ARM constant scan over %d modules in %s' % (len(names), root))
    print('# looking for %s' % ', '.join(str(x) for x in wanted))
    print('#')
    print('# encodability as an ARM data-processing immediate:')
    for c in wanted:
        e = arm_encodable(c)
        print('#   %5d (0x%03X)  %s' % (c, c, ('YES  0x%02X ror #%d' % e) if e
                                        else 'NO   -- literal-pool word only'))
    print('')
    hdr = ('%-12s %10s %10s %10s %10s %8s %8s %8s'
           % ('module', 'ARM imm', 'THUMB lit', 'words', 'ldr targets',
              'imm hit', 'thm hit', 'word hit'))
    print(hdr)
    print('-' * len(hdr))
    tot = dict(imm_total=0, thumb_total=0, word_total=0, load_targets=0,
               imm=0, thm=0, word=0)
    detail = []
    for n in names:
        body = open(os.path.join(root, n), 'rb').read()
        r = arm_scan(body, 0, wanted)
        print('%-12s %10d %10d %10d %10d %8d %8d %8d'
              % (n[:-4], r['imm_total'], r['thumb_total'], r['word_total'],
                 r['load_targets'], len(r['imm_hits']), len(r['thumb_hits']),
                 len(r['lit_hits'])))
        tot['imm_total'] += r['imm_total']
        tot['thumb_total'] += r['thumb_total']
        tot['word_total'] += r['word_total']
        tot['load_targets'] += r['load_targets']
        tot['imm'] += len(r['imm_hits'])
        tot['thm'] += len(r['thumb_hits'])
        tot['word'] += len(r['lit_hits'])
        for va, w, name, val in r['imm_hits']:
            detail.append((n[:-4], 'ARM immediate', va, w, name, val, None))
        for va, w, name, val in r['thumb_hits']:
            detail.append((n[:-4], 'THUMB immediate', va, w, name, val,
                           struct.unpack_from('<I', body, (va & ~3))[0]))
        for va, w, refs in r['lit_hits']:
            detail.append((n[:-4], 'literal-pool word', va, w, '', w,
                           refs if refs else None))
    print('-' * len(hdr))
    print('%-12s %10d %10d %10d %10d %8d %8d %8d'
          % ('TOTAL', tot['imm_total'], tot['thumb_total'], tot['word_total'],
             tot['load_targets'], tot['imm'], tot['thm'], tot['word']))
    print('')
    if not detail:
        print('Not one of %s in either ARM encoding, anywhere in %d modules.'
              % (', '.join(str(x) for x in wanted), len(names)))
        print('Section 7: the absence of these constants is evidence the decoder')
        print('is not present rather than evidence it was not found -- and on ARM')
        print('the literal-pool pass is the half that matters, because 4070, 4071,')
        print('4078 and 4079 cannot be written as immediates at all.')
        return 0
    print('# every hit, so that none is quoted without being read')
    for mod, kind, va, w, name, val, extra in detail:
        print('  %-12s %-18s +0x%06X  word 0x%08X  value %d  %s'
              % (mod, kind, va, w, val, name))
        if kind == 'THUMB immediate':
            print('      containing ARM word at +0x%06X = 0x%08X -- section 7 '
                  'requires this be read before the hit is counted'
                  % (va & ~3, extra))
        if kind == 'literal-pool word':
            if extra:
                print('      loaded by: %s'
                      % ', '.join('%s ldr @+0x%06X' % (k, a) for k, a in extra[:6]))
            else:
                print('      NOT the target of any PC-relative load in this module')
    return 0
...
def main(argv):
    ...
    data = b'' if '--dir' in argv else open(path, 'rb').read()
