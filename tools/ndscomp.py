...
def blz_decompress(data):
    ...
    The layout, from the eight-byte footer backwards:

      -8  u24 enc_len     bytes at the end of the file that are encoded,
                          *including* the footer
      -5  u8  hdr_len     size of that footer, 8 to 11 -- 11 when the linker
                          padded it
      -4  u32 inc_len     how much longer the plaintext is

    so `dec_len = filelen - enc_len` bytes at the front are verbatim, the
    `enc_len - hdr_len` bytes after them are an LZSS stream written
    **backwards**, and the plaintext is `filelen + inc_len` bytes long.

    Two things about this routine were wrong until a cartridge with packed
    modules turned up to check it against.  *Tales of the Tempest* has no
    overlays and *Tales of Innocence*'s three are stored plain, so in two DS
    pipelines nothing here was ever executed:

      * **the two bytes of a match token were assembled in the wrong order.**
        Walking backwards, the byte at the *higher* address is the one the
        forward reader sees first and therefore the high half of the token.
      * **the copy was not clamped to the end of the encoded region**, so a
        match that runs past it corrupted the verbatim prefix instead of
        being truncated the way the reference implementation truncates it.

    Both failures come out as an exception or as a plaintext that is not code.
    """
    if len(data) < 8:
        raise Bad('too short')
    ...
    if enc_len > len(data) or enc_len < hdr_len:
        raise Bad('encoded length %d beyond the file' % enc_len)
    dec_len = len(data) - enc_len          # verbatim prefix
    pak_len = enc_len - hdr_len            # the encoded stream itself
    raw_len = len(data) + inc_len          # the plaintext

    pak = bytes(reversed(data[dec_len:dec_len + pak_len]))
    out = bytearray(raw_len - dec_len)
    pi = 0
    ri = 0
    mask = 0
    flags = 0
    ...
    while ri < n:
        ...
        if not flags & mask:
            ...
            if ri + ln > n:
                ln = n - ri
            if pos > ri:
                raise Bad('back-reference before the start of the output')
            for _ in range(ln):
                out[ri] = out[ri - pos]
                ri += 1
    if ri != n:
        raise Bad('stream ended %d bytes short of the declared plaintext' % (n - ri))
    return data[:dec_len] + bytes(reversed(out))
