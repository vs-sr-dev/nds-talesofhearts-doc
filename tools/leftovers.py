...
It also answers, separately and always, the question the corpus keeps having to
re-ask: **does this build name who made it?**  The three DS cartridges give
three different answers -- *Tempest* names nobody in any alphabet, *Innocence*
names Alfa System three ways, and this one had to be checked in a third script
before the fourteenth build's Shift-JIS pass was folded in here.  The company
search runs in **ASCII, Shift-JIS and UTF-16LE**, because a title that names
nobody in the first may name everybody in the second, and the DS banner is
UTF-16.  Every needle is printed with its count including the zeros, so a
silent category is visible as a zero rather than as an omission.

    python leftovers.py FILE [FILE...] [--min 6] [--all] [--jis]
    python leftovers.py --tree DIR --rom IMAGE
    python leftovers.py --who FILE [FILE...]
...
COMPANY = ['namco', 'bandai', 'namcobandai', 'namco bandai', 'wolfteam',
           'wolf team', 'tales studio', 'talesstudio', 'alfa system',
           'alfasystem', 'ganbarion', 'dimps', 'telenet', 'nintendo',
           'hudson', 'criware', 'cri middleware', 'cri ', 'actimagine',
           'mobiclip', 'monolith', 'tri-crescendo', 'tri-ace']

# The same question in the alphabet the target actually writes in.
COMPANY_JP = [
    ('\u30ca\u30e0\u30b3', 'Namco'),
    ('\u30d0\u30f3\u30c0\u30a4', 'Bandai'),
    ('\u30d0\u30f3\u30c0\u30a4\u30ca\u30e0\u30b3', 'Bandai Namco'),
    ('\u30c6\u30a4\u30eb\u30ba\u30b9\u30bf\u30b8\u30aa', 'Tales Studio'),
    ('\u30a6\u30eb\u30d5\u30c1\u30fc\u30e0', 'Wolf Team'),
    ('\u682a\u5f0f\u4f1a\u793e', 'K.K. / Inc.'),
    ('\u30c6\u30a4\u30eb\u30ba', 'Tales'),
    ('\u30cf\u30fc\u30c4', 'Hearts'),
    ('\u30a2\u30eb\u30d5\u30a1\u30fb\u30b7\u30b9\u30c6\u30e0', 'Alfa System'),
    ('\u30b9\u30bf\u30c3\u30d5', 'Staff'),
]

CLASSNAME = re.compile(rb'[A-Za-z_][A-Za-z0-9_]{2,40}(?:::[A-Za-z_][A-Za-z0-9_]{2,40})+')
# The Innocence framework spelled its classes `cMappyComponentDSStandardEntity`
# -- a lower-case tag letter and then CamelCase -- which is how a build with
# RTTI on gives its framework away without any `::` in the name at all.
CTYPE = re.compile(rb'\b[a-z][A-Z][A-Za-z0-9]{6,60}\b')


def whole_string(b, lo, hi):
    """Is [lo,hi) a complete C string rather than a run inside binary data?

    A name in a symbol table or an RTTI record is a whole token: the bytes on
    either side of it are not identifier characters.  A regular expression that
    does not require this matches inside compressed payload, and on a 256 MiB
    cartridge it matches thousands of times.

    The delimiter is deliberately "not an identifier character" rather than
    "NUL or whitespace": this build separates its runtime type names with `!`,
    and the stricter test threw away the only real names on the cartridge.
    """
    ident = set(b'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_:')
    before = b[lo - 1] if lo else 0
    after = b[hi] if hi < len(b) else 0
    return before not in ident and after not in ident


def cmd_who(paths):
    """Does this build name anybody, in any of the three alphabets?"""
    print('=' * 72)
    print('who made it: every needle, in ASCII, Shift-JIS and UTF-16LE')
    print('=' * 72)
    print('')
    blobs = []
    for p in paths:
    ...
    print('-- company and middleware names, ASCII (case-insensitive) --')
    for c in COMPANY:
        n = 0
        where = []
        for name, b in blobs:
            k = b.lower().count(c.encode('ascii'))
            n += k
            if k:
                where.append('%s x%d' % (name, k))
        print('   %-20s %5d   %s' % (c, n, ', '.join(where[:6])))
    print('')

    for label, enc in (('Shift-JIS', 'shift_jis'), ('UTF-16LE', 'utf-16-le')):
        print('-- company names, %s --' % label)
        for jp, meaning in COMPANY_JP:
            try:
                needle = jp.encode(enc)
            ...
            for name, b in blobs:
                ...
                if k:
                    where.append('%s x%d' % (name, k))
            print('   %-14s %-14s %5d   %s'
                  % (jp, meaning, n, ', '.join(where[:6])))
        print('')

    print('-- C++ class names --')
    print('   A build with RTTI left on carries demangled class names; one')
    print('   with it off carries none, and that is a fact about the build')
    print('   settings rather than about the studio.  Innocence 2007 has')
    print('   1,047, Vesperia 2008 has 445, Ratatosk 2008 has 0.')
    print('')
    print('   A match is only counted when it is a whole NUL- or')
    print('   whitespace-delimited C string.  Without that, a regular')
    print('   expression run over compressed payload invents thousands:')
    print('   the unrestricted form finds 3,240 CamelCase identifiers on')
    print('   this cartridge and every one of them is inside a compressed')
    print('   or high-entropy buffer.')
    print('')
    seen = {}
    tagged = {}
    for name, b in blobs:
        for m in CLASSNAME.finditer(b):
            if not whole_string(b, m.start(), m.end()):
                continue
            seen.setdefault(m.group().decode('latin1'), set()).add(name)
        for m in CTYPE.finditer(b):
            if not whole_string(b, m.start(), m.end()):
                continue
            tagged.setdefault(m.group().decode('latin1'), set()).add(name)
    print('   %5d distinct `A::B` qualified names, whole strings' % len(seen))
    for x in sorted(seen):
        print('        %-40s %s' % (x, ', '.join(sorted(seen[x]))[:60]))
    print('   %5d distinct tag-letter CamelCase identifiers, whole strings'
          % len(tagged))
    for x in sorted(tagged)[:60]:
        print('        %-40s %s' % (x, ', '.join(sorted(tagged[x]))[:60]))
    return 0


def main(argv):
    if '--who' in argv:
        return cmd_who([a for a in argv[1:] if a != '--who'])
    if '--tree' in argv:
