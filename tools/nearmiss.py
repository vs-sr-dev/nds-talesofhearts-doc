...
TRIG_HEAD = """
   * what the 4080 sites actually are, form 1: constant-returning stubs
     On *Tales of the Tempest* the `mov r0,#4080` instructions all sat in one
     region of eight-byte stubs -- `mov r0,#K ; bx lr` or `ldr r0,[pc,..] ;
     bx lr` -- reached by a table of `b` instructions above them, which is
     the ARM computed-branch idiom.  What they returned was a 4,096-scaled
     trigonometric table in whole degrees."""

TRIG_TABLE_HEAD = """
   * what the 4080 sites actually are, form 2: a plain table of words
     The same table can be emitted as an ordinary array rather than as
     compiled stubs, and then a literal-pool hit sits inside it and no
     PC-relative load points at any individual entry.  For every unreferenced
     4080 word this checks whether the words around it are
     round(4096 * cos theta) for whole degrees, which is what
     `round(4096 * cos 5 deg) = 4080` makes likely."""


def trig_run(data, off, span=48):
    """Is the aligned-word run containing `off` a 4,096-scaled cosine table?

    Returns (matched, checked, first_index, values) where `first_index` is the
    angle in degrees the run appears to start at.  A table stored as sine or
    with a different phase still matches, at a different starting index.
    """
    lo = max(0, off - span * 4)
    hi = min(len(data) - 3, off + span * 4)
    vals = []
    for i in range(lo - lo % 4, hi, 4):
        vals.append(struct.unpack_from('<i', data, i)[0])
    if not vals:
        return 0, 0, None, []
    best = (0, None)
    for start in range(-180, 181):
        m = 0
        for k, v in enumerate(vals):
            if abs(v - round(4096 * math.cos(math.radians(start + k)))) <= 1:
                m += 1
        if m > best[0]:
            best = (m, start)
    return best[0], len(vals), best[1], vals
...
def main(argv):
    ...
    if not vals:
        print('     (this image has none: the 4080 sites here are not stubs)')
    if vals:
    ...
    print(TRIG_TABLE_HEAD)
    unref = [va for va, w, refs in lit_hits if not refs]
    if not unref:
        print('     no unreferenced literal-pool hit to test')
    for va in unref:
        m, n, start, vv = trig_run(data, va - base)
        print('     word at 0x%08X: %d of %d surrounding aligned words are'
              % (va, m, n))
        print('       round(4096 * cos theta) to within one, for theta starting'
              ' at %s degrees' % start)
        print('       run: %s' % ', '.join(str(x) for x in vv[:16]))
    print('     round(4096 * cos 5 deg) = %d'
          % round(4096 * math.cos(math.radians(5))))
